...
namespace MTDBFrameworkBase.Data
{
    ...
    public class ConsensusTarget : INotifyPropertyChanged, IComparable
    {
        ...
        /// The Prefix Residue
        /// </summary>
        public char PrefixResidue
        {
            get => string.IsNullOrWhiteSpace(m_sequence) ? '\0' : m_sequence.First();
            // ReSharper disable once ValueParameterNotUsed
            // ReSharper disable once UnusedMember.Local
            private set { /* Empty setter; setter required for database reading */ }
        }

        /// <summary>
        /// The Suffix Residue
        /// </summary>
        public char SuffixResidue
        {
            get => string.IsNullOrWhiteSpace(m_sequence) ? '\0' : m_sequence.Last();
            // ReSharper disable once ValueParameterNotUsed
            // ReSharper disable once UnusedMember.Local
            private set { /* Empty setter; setter required for database reading */ }
        }
        ...
        {
            var massesList = Evidences.Select(c => c.MonoisotopicMass).ToList<double>();
            var netList = Evidences.Select(c => c.ObservedNet).ToList<double>();
