...
{
	clsCentralRegression::clsCentralRegression(void)
	{
		mint_num_x_bins = 0 ; 
		mint_num_y_bins = 0 ; 
		mint_num_jumps = 0 ; 
		mdbl_tolerance = 0.8 ; // 5 standard deviations.
		mdbl_outlier_zscore = mdbl_tolerance ; 
	...
	void clsCentralRegression::SetOptions(int num_x_bins, int num_y_bins, int num_jumps, double ztolerance)
	{
		mint_num_x_bins = num_x_bins ; 
		mint_num_y_bins = num_y_bins ; 
		mint_num_jumps = num_jumps ; 
		mdbl_tolerance = ztolerance ; 
		mdbl_outlier_zscore = ztolerance ; 
	...
	void clsCentralRegression::SetOutlierZScore(double outlier_zscore)
	{
		mdbl_outlier_zscore = outlier_zscore ; 
	}

	void clsCentralRegression::CalculateMinMax()
	{
		...
		mdbl_min_y = DBL_MAX ; 
		mdbl_max_y = -1 * DBL_MAX ; 

		for (int i = 0 ; i < num_pts ; i++)
		{
			clsRegressionPts pt = mvect_pts[i] ; 
			if (pt.mdbl_x < mdbl_min_x)
				mdbl_min_x = pt.mdbl_x ; 
			if (pt.mdbl_x > mdbl_max_x)
				mdbl_max_x = pt.mdbl_x ; 
			if (pt.mdbl_y < mdbl_min_y)
				mdbl_min_y = pt.mdbl_y ; 
			if (pt.mdbl_y > mdbl_max_y)
				mdbl_max_y = pt.mdbl_y ; 
		}
	}


	void clsCentralRegression::CalculateSectionStdAndCount(int interval_num)
	{
		vector<double> vect_pts ;

		int num_pts = mvect_pts.size() ; 
		double x_interval = (mdbl_max_x - mdbl_min_x)/ mint_num_x_bins ;

		for (int pt_num = 0 ; pt_num < num_pts ; pt_num++)
		{
			clsRegressionPts pt = mvect_pts[pt_num] ; 
			int section_num = (pt.mdbl_x - mdbl_min_x) / x_interval ; 
			if (section_num == mint_num_x_bins)
				section_num = mint_num_x_bins - 1 ; 
			if (interval_num == section_num)
			{
				mvect_count[section_num]++ ; 
				vect_pts.push_back(pt.mdbl_y) ; 
			}
		}
...
//				double mismatch_score = (tolerance * tolerance) / (mvect_std_y[interval_num]*mvect_std_y[interval_num]) ; 
				double mismatch_score = mdbl_tolerance * mdbl_tolerance ; 
				mvect_section_mismatch_score[interval_num] = mismatch_score ; 
			}
			else
			{
				mvect_section_mismatch_score[interval_num] = mdbl_tolerance * mdbl_tolerance ; 
	...
	void clsCentralRegression::CalculateSectionsStd()
	{
	...
	void clsCentralRegression::Clear()
	{
		mvect_match_scores.clear() ; 
		mvect_alignment_scores.clear() ; 
		mvect_best_previous_index.clear() ; 
	...
	void clsCentralRegression::SetUnmatchedScoreMatrix()
	{
		mvect_match_scores.clear() ; 
	...
	void clsCentralRegression::CalculateScoreMatrix()
	{
		...
		double y_interval_size = (mdbl_max_y - mdbl_min_y)/ mint_num_y_bins ;
		double x_interval_size = (mdbl_max_x - mdbl_min_x)/ mint_num_x_bins ;
		...
		{
			clsRegressionPts pt = mvect_pts[pt_num] ; 
			int x_section = (pt.mdbl_x - mdbl_min_x) / x_interval_size ; 
			...
			// numeric issues...
			int y_interval = (0.0001 + (pt.mdbl_y - mdbl_min_y) / y_interval_size) ; 
			...
			int min_y_start = y_interval - y_tolerance / y_interval_size ; 
			int max_y_start = y_interval + y_tolerance / y_interval_size  ; 

			double section_mismatch_score = mvect_section_mismatch_score[x_section] ; 

			double x_fraction = (pt.mdbl_x - mdbl_min_x) / x_interval_size - x_section ; 
			...
			for (int y_from = min_y_start ; y_from <= max_y_start ; y_from++)
			{
				...
				for (int y_to = y_from - mint_num_jumps ; y_to <= y_from + mint_num_jumps ; y_to++)
				{
					...
					y_estimated = (y_from + (y_to - y_from) * x_fraction) * y_interval_size + mdbl_min_y ; 
					double y_delta = pt.mdbl_y - y_estimated ; 
	...
	void clsCentralRegression::CalculateAlignmentMatrix()
	{
		mvect_best_previous_index.clear() ;
		mvect_alignment_scores.clear()  ; 
	...
	void clsCentralRegression::CalculateRegressionFunction()
	{
		mmap_alignment_function.clear() ; 
	...
	void clsCentralRegression::RemoveOutliersAndCopy(vector<clsRegressionPts> &calib_matches)
	{
		mvect_pts.insert(mvect_pts.begin(), calib_matches.begin(), calib_matches.end()) ; 
	}

	void clsCentralRegression::CalculateRegressionFunction(vector<clsRegressionPts> 
															&calib_matches)
	{
		Clear() ; 
	...
	void clsCentralRegression::PrintScoreMatrix(char *file_name)
	{
		double y_interval_size = (mdbl_max_y - mdbl_min_y)/ mint_num_y_bins ;
		double x_interval_size = (mdbl_max_x - mdbl_min_x)/ mint_num_x_bins ;

		ofstream fout(file_name) ; 
		double x_start = mdbl_min_x ;
		for (int x_section = 0 ; x_section < mint_num_x_bins ; x_section++)
		{
			...
			for (int y_section = 0 ; y_section < mint_num_y_bins ; y_section++)
			{
				for (int jump = 0 ; jump < 2*mint_num_jumps+1 ; jump++)
				{
					...
					fout<<x_start<<","<<x_stop<<"," ; 
					fout<<y_section*y_interval_size+mdbl_min_y<<","<<(y_section+jump-mint_num_jumps)*y_interval_size+mdbl_min_y<<","<<mvect_match_scores[index]<<"\n" ; 
				}
			}
			x_start = x_stop ; 
		}
	...
	void clsCentralRegression::PrintAlignmentScoreMatrix(char *file_name)
	{
		double y_interval_size = (mdbl_max_y - mdbl_min_y)/ mint_num_y_bins ;
		double x_interval_size = (mdbl_max_x - mdbl_min_x)/ mint_num_x_bins ;
...
//					fout<<x_section<<","<<y_section<<","<<y_section-jump<<mvect_match_scores[index]<<"\n" ; 
					fout<<x_start<<","<<x_stop<<"," ; 
					fout<<y_section*y_interval_size+mdbl_min_y<<","<<mvect_alignment_scores[index]<<"\n" ; 
			}
			x_start = x_stop ; 
		}
		fout.close() ; 
	...
	void clsCentralRegression::PrintRegressionFunction(char *file_name) 
	{
		double y_interval_size = (mdbl_max_y - mdbl_min_y)/ mint_num_y_bins ;
		double x_interval_size = (mdbl_max_x - mdbl_min_x)/ mint_num_x_bins ;
			...
			iter++)
		{
			...
			fout<<x_start<<","<<y_section*y_interval_size+mdbl_min_y<<"\n" ; 
			x_start = x_stop ; 
		}
		fout.close() ; 
	}
	...
	void clsCentralRegression::PrintPoints(char *file_name)
	{
		ofstream fout(file_name) ; 
		for (int pt_num = 0 ; pt_num < mvect_pts.size() ; pt_num++)
		{
			clsRegressionPts pt = mvect_pts[pt_num] ; 
			fout<<pt.mdbl_x<<","<<pt.mdbl_y<<"\n" ; 
		}
		fout.close() ; 
	}

	void clsCentralRegression::RemoveRegressionOutliers()
	{
		double x_interval_size = (mdbl_max_x - mdbl_min_x)/ mint_num_x_bins ;
		vector<clsRegressionPts> vect_temp_pts ; 
		int num_pts = mvect_pts.size() ; 
		for (int pt_num = 0 ; pt_num < num_pts ; pt_num++)
		{
			clsRegressionPts pt = mvect_pts[pt_num] ; 
			int interval_num = (pt.mdbl_x - mdbl_min_x) / x_interval_size ; 
			if (interval_num == mint_num_x_bins)
				interval_num = mint_num_x_bins - 1 ; 
			double std_y = mvect_std_y[interval_num] ; 
			double val = GetPredictedValue(pt.mdbl_x) ; 
			double delta = (val - pt.mdbl_y) / std_y ; 
			if (abs(delta) < mdbl_outlier_zscore) 
			{
	...
	double clsCentralRegression::GetPredictedValue(double x)
	{
		double y_interval_size = (mdbl_max_y - mdbl_min_y)/ mint_num_y_bins ;
		double x_interval_size = (mdbl_max_x - mdbl_min_x)/ mint_num_x_bins ;
		int x_section = (x - mdbl_min_x) / x_interval_size ; 
		if (x_section >= mint_num_x_bins)
			x_section = mint_num_x_bins - 1 ; 
		if (x_section < 0)
		{
			...
			return mdbl_min_y + (*iter).second * y_interval_size ; 
		}

		double x_fraction = (x - mdbl_min_x) / x_interval_size - x_section ; 
		...
		double y_pred = x_fraction * y_interval_size * (y_section_to - y_section_from)
									+ y_section_from * y_interval_size + mdbl_min_y ; 
